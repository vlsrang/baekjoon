...
def find_n_nearest_value(X, Y, new_x, k):
    ...
    if k==0:
        print("The k is not zero!!")
        return None
    dists=[math.sqrt((x-new_x)**2) for x in X]
    ...
    for i in range(num):
        for j in range(i,num):
            if dists[j]<dists[i]:
                dists[j], dists[i] = dists[i], dists[j]
                X[j], X[i] = X[i], X[j]
                Y[j], Y[i] = Y[i], Y[j]
    pred_y=0
    for y in Y[:k]:
...
'''    
    ...
    def dist(new_x, X):
        ...
        for i in range(len(new_x)):
            distance += (new_x[i] - X[i])**2
            return math.sqrt(distance)

    new_x=[0.5]
    distance=[] # 거리 산출
    for X in Z:
        distance.append(dist(new_x, X))

                
    for i in range(len(Z)):
        Z[i].append(distance[i])
    ...
    x_values=[] # 가장 가까운 k개의 x좌표 산출
    for j in range(k):
        x_values.append(Z[j][0])
 
    fx_values=[] # F(x)값들의 합 산출
    pred_y=0
    for i in range(k):

        fx_values.append(Z[i][1])
        pred_y += Z[i][1]
'''
